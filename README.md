...
|---|---|---|---|
| 1 | Good Prompt Techniques | Agents Definition Cell | Configured strict Role, Context, Task, Constraints, and structured markdown outputs for all 3 agents. |
| 2 | Control-Flow Pattern | Routing & Execution Loop Cell | Implemented an LLM classifier router (`url_classifier`) to evaluate incoming links and restrict processing only to valid human-disease Wikipedia URLs. |
| 3 | CrewAI Crew | Tasks & Crew Definition Cells | Implemented 3 specialist agents with explicit context handoffs (`gene_task` $\rightarrow$ `matcher_task` $\rightarrow$ `writing_task`). |
| 4 | Custom & Prebuilt Tools | Tools Definition Cell | Integrated a prebuilt `ScrapeWebsiteTool` for static web parsing and a custom `@tool` (`search_clinical_trials`) wrapping the live ClinicalTrials.gov REST API. |
| 5 | Planning / Flow | Planning & Execution Cell | Enabled `planning=True` alongside an explicit planning LLM handler to map out tasks before execution. |
| 6 | Memory | Crew Instantiation Cell | Configured `memory=True` using the `google-generativeai` provider with `gemini-embedding-001`, tested via dual execution passes. |
